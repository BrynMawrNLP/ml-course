...
$$\lambda = [0,10^8,10^7,\ldots,10^0]$$ so that $$\lambda$$ is on a logistic scale, with regularization increasing as $$x$$ increases). How does regularization a↵ect training and test error? Which $$\lambda$$ value appears to work best? writeup
